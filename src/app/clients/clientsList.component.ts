...
export class ClientsListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.appServices.getLang('pt_br', 'clients')
    ...
    this.listService.onSearchRequest("searchString")
      .subscribe(data => this.searchClients(data));

    this.clientsService.getClients(this._lastName, this._pageSize * 3)
      .subscribe(usrs => {
        if (!usrs)
       ...
       .subscribe(usrs =>
         {
          if(usrs.length>0) {
             ...
             this.rows        = this.listService.paginate();
             this.client        = this.rows[this.rows.length-1];
             this._lastName   = this.client.name;
          } else {
  ...
  deleteUsers() {
    ...
    return this.clientsService.deleteClient(deleteList)
      .subscribe(usr => {
  ...
  searchClients(searchString: string) {
    ...
    this.clientsService.searchClientsByName(this.searchString)
      .subscribe(usrs =>
        {
          this.listService.loadRows(usrs, true);
          this.rows = this.listService.paginate();
